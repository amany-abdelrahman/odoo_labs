...
class ResPartner(models.Model):
    ...
    def _check_unique_email(self):
        for rec in self:
            if rec.email and rec.env['patient'].search([('email', '=', rec.email)]):
                raise ValidationError('Email address already exists in the patient model.')
...
class ResPartner(models.Model):
    ...
    def _check_linked_customer(self):
        for rec in self:
            if rec.patient_id:
